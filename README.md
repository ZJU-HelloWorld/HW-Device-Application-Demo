...
* 采用USB传输时，修改了由STM32CubeMX生成的`usbd_cdc_if.c/h`文件中`CDC_Receive_FS`函数用于自定义数据接收；修改了`usb_device.c`中的`MX_USB_DEVICE_Init`函数，将DP引脚拉低一段时间后再进行一般的初始化，使得Reset后不需要重新插拔USB数据线即可使上位机重新识别到设备.
---------------
